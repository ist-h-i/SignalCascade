...
- 信頼度が中以下のときは、強い CTA よりも `保留理由` や `様子見 / Wait` を優先する。
...
- 日本語は `意味が一読で入る短語` を優先し、直訳調の語を避ける。
- `観察継続`、`再利用候補`、`取りにいく`、`確認待ち` のような内部メモ調の語は UI に出さない。
- 判断語彙は `強気`、`弱気`、`中立`、`様子見`、`見送り`、または `Bullish`、`Bearish`、`Neutral`、`Wait`、`Go`、`Pass` のような短語を優先する。
- 第一視界のテキスト量は `大見出し 1`、`補助文 1`、`主要ファクト 3-4` を上限の目安にする。
- 重要な意味は文章より先に `アイコン / ピクト / 状態色 / 位置` で伝える。
- 画面全体に小さなラベルを散らさない。ラベルが判断を助けないなら削る。
- source path、schema version、内部 status のような運用メタ情報は第一視界に置かない。
- 各パネルの補助文は 1 行までを基本とし、複数文で説明しない。
